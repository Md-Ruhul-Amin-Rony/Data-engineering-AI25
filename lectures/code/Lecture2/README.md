...
* **COPY order.** Copy the source before installing dependencies and every
  one-character edit re-runs `pip install`.
...
rebuild, and watch pip download the internet again. Put it back.
...
| build is slow every time | `COPY . .` before `pip install`, or no `.dockerignore` |
